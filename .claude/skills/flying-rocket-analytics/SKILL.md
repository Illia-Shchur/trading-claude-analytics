...
2. **Compute:** `node tools/compute.mjs fr-funding --per8h X` (annualized + monthly bleed, sign convention printed), `fr-cap --spot X --ath1y Y` (cycle cap tier), `band fr-*` (rubric bands), `ev`, `adr`, `squeeze --funding-annualized X [--sustained3] [--oi-within-5pct] [--single-below-7]` (the §4 squeeze-trap penalty — Channel B gate 8, the only gate in either framework that voids an unlock on its own; take `--sustained3` from `tools/fetch.mjs`'s `funding.sustained3_below_minus5`, never `longest_negative_run_intervals`), `short-ev --directional-ev X --funding-annualized X --hold-days N [--target-gain-pct N]` (§5/§6 — the carry zero-floor, the +3% minimum-edge filter, and the 40%-of-target carry veto, all in one call). **Edge convention codified in code (Hard Rule 6):** where §4's dash-range bands leave an exact edge ambiguous, `tools/lib.mjs` resolves it to the LOWER-score band — the harder-to-short reading (RSI exactly 75 → 3, not 4; MVRV-Z exactly 5 → 4; exactly 10% below 1y-ATH → cap 14 applies). A tightening-or-neutral mirror of the FK edge rule, never a loosening.
...
4. **Context Panel (market-data-extension plan, 2026-08, optional):** derivatives positioning that used to be pure web-fetch narration is now partly tool-computed under `outp.context` — perp basis/carry (`context.basis`), Binance long/short + taker + OI (`context.positioning`), (BTC/ETH only) Deribit ATM IV/moneyness skew/VRP (`context.deribit` — sign convention 2026-08-05: POSITIVE = the ~10% OTM put is richer than the ~10% OTM call = a downside hedging bid; a blow-off COMPRESSES or INVERTS this skew, never richens it), and (BTC/ETH/SOL only) Bitfinex spot borrow (`context.borrow` — single venue, a lending book not necessarily the short's actual borrow venue, frequently thin) — plus realized-vol/drawdown/RSI percentiles (now including the DAILY RSI and bounce-% percentiles Channel B's own legs score, and the phase-of-cycle cap's `high_1y.pct_below` percentile) and macro-scope `net_liquidity`/`hy_oas`/`nfci`/`stablecoin_supply` from `tools/fetch.mjs macro`. **DISCLOSED CONTEXT ONLY** — none of this is a scored leg, gate, threshold, phase size, stop, or cap, and it does not relax the Hard Rule 1 web-fetch requirement for ETF flows/on-chain/news above. `tools/tripwire.mjs` (run by hand, not part of the mandatory workflow) can flag a scoring-relevant boundary crossing between two stored `tools/snapshot.mjs` runs without a full report — including, as of 2026-08-05, every §4A/§4B band edge and the actual gate-8 funding boundary, not just the informational funding-sign flip it started with.
...
2. **Where a gate names a quantity no leg defines and `tools/lib.mjs` emits a field of that name, the tool's field is primary** and is cited by name in the report. This settles gate 2 on `trend.bounce_age_sessions`, and it is why `dailyTrend()` emits `sessions_low_to_high` alongside rather than discarding it.
...
**Equity indices and metals are NOT in this framework's scope, and this annex does not put them in it.** The `description`, the Overview asset-scope line and the Asset Generalization table above are unchanged and remain crypto-only. This annex exists because such reports get requested anyway — five have been produced (gold ×2, SPX ×2, and the gold pair's companions) — and the ad hoc regime demonstrably drifted: four reports, four gate denominators, two rounding conventions, and two mutually contradictory valuation readings on the same asset on the same day. **The annex formalizes the restrictions, not the coverage.** Every rule below is a tightening or a disclosure; none makes a non-crypto short easier than the ad hoc practice it replaces.
