...
6. **Toolchain sanity.** Run `node tools/selftest.mjs` before launching the workflow — the frameworks' reports are now built on `tools/` computations, and a broken toolchain invalidates every downstream number. Reports from Jul 2026 on end with a ` ```json machine ` block (schema `report-machine/1`): extraction agents treat it as authoritative for the numeric fields, which makes extraction on modern reports cheaper and mis-extraction rarer; `node tools/lint-report.mjs <report> --legacy` can spot-check any report's arithmetic during grading.
