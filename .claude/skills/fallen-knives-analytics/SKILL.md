...
5. **ONE-TIME (added 2026-08-03, delete this line once discharged): immediately after this report is saved and committed, run commit 12 of the 2026-08 toolchain-extension plan** — the spot-canonical flip, step B (`tools/fetch.mjs`: `spot.canonical` becomes the median from `spot.panel`, `spot.canonical_median` is kept as a deprecated echo). Step A (commit 7, already shipped) added the panel without touching `canonical`; this report is the first live run under step A, so it is the trigger. State the observed delta (gate-6 boolean, cap tier) in commit 12's own commit message, then remove this numbered rule from the SKILL.
6. **Context Panel (market-data-extension plan, 2026-08, optional):** `tools/fetch.mjs`'s `outp.context` block — realized-vol/drawdown/RSI/200dma/F&G/funding percentiles vs each metric's own recent history, plus (BTC/ETH only) `context.deribit` options vol surface, `context.basis`/`context.positioning`, and macro-scope `net_liquidity`/`hy_oas`/`nfci`/`stablecoin_supply` from `tools/fetch.mjs macro`. **DISCLOSED CONTEXT ONLY** — none of this is a scored leg, gate, threshold, phase size, stop, or cap, and it never substitutes for a Required Data Fetch above. `tools/tripwire.mjs` (run by hand, not part of the mandatory workflow) can flag a scoring-relevant boundary crossing between two stored `tools/snapshot.mjs` runs without a full report.
