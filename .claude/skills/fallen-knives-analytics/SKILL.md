...
- **Frequency:** at most one conviction-path unlock per report; a phase may not be re-unlocked through any discretionary channel within **10 calendar days** of a discretionary stop-out (D5).
- The D5 stop applies.
...
- **On a hit:** exit that tranche in full, return the capital to dry powder, log the realized loss in the Discretion Ledger, and bar discretionary channels for that phase for **10 calendar days**.
- The catastrophic floor and the §7 narrative-break exit continue to govern the whole book independently.
- **Discretionary capital cap:** capital deployed through **all non-mechanical channels combined — D1 crosses, D2 conviction paths, and Override firings — may not exceed 40% of the book** until a [T] gate relights or a confirmed higher-low prints. The Deep-Value Override keeps its own tighter **≤25% sub-cap, counted inside that 40%** (unchanged, not additive to it).
...
2. **First-time calibration is not a migration.** Setting a parameter for the first time in a series — including the per-asset compound score line for a structurally-pinned-score asset, which must be re-set below that asset's realized score range at stop-set time — is an initial calibration, not a widening. Once set, it ratchets.
...
Round to nearest integer. **Adjusted score = round(Σ legs + the D1 discretionary term) — no multiplicative modifier remains (Jun 2026); the [V]-gate surcharge changes gate requirements only, never the score number.** The D1 term is the framework's only additive analyst input: state it every report (0 counts), with its rationale, falsifier, and Ledger entry per the Analyst Discretion Layer. Print the leg sum, the D1 term, the raw composite, any [V]-gate surcharge, and the adjusted score as separate numbers so the mechanical and discretionary halves stay auditable. Half-point ties follow the established per-asset convention (adjudicated in the 2026-07-04 calibration — see the "round-down-at-.5" rejection): BTC and Gold round .5 **up**; ETH rounds .5 **down** (estimate-heavy input set → conservative on a buy signal). For a new asset, declare a convention with a one-line rationale at its first .5 occurrence and hold it constant for the series. State raw score, any [V]-gate surcharge applied, and adjusted score. *(The Flying Rocket ×1.05 decoupled modifier is demoted symmetrically — see that skill's revision log.)*
...
- **Stop Migration Ledger (Jul 2026).** Any change to any stop parameter (price line, score condition, checkpoint date, catastrophic anchor) prints a Stop Migration line: old → new, direction (toward/away from price), which TIER changed (compound-line vs catastrophic-floor vs score-condition vs checkpoint date — named explicitly), and one-line rationale. Each parameter that changes value gets its own line even when multiple move in the same report. This applies to every migration regardless of direction — "silent" is defined as any un-annotated numeric change to a stop parameter. Silent migrations are a publishing FAIL. **Superseded in part 2026-07-27:** the Ledger still *records* every change, but the **D6 ratchet** now decides which changes are *permitted at all* — a migration away from price is prohibited outright (not merely disclosable), save the single named exception for re-anchoring the catastrophic floor onto a previously-named deeper zone. A loosening that reverses a prior tightening is no longer a disclosable event; it is a rejected one.
...
|---|---|---|
| Adjusted score drops ≥6 points from local peak — *local peak* = the highest adjusted score printed since the current campaign's first fill (a cold start resets it). **Measured on the mechanical leg sum, excluding the D1 discretionary term (2026-07-27)** — otherwise retiring a +2 adjustment would itself manufacture a third of a trim trigger. Labeled measurement/input-honesty corrections do NOT count toward the drop: restate the peak on corrected inputs and compare like-for-like (carve-out valid only when the correction was labeled in the report where it was taken) | Trim 25% | Signal exhaustion in progress |
