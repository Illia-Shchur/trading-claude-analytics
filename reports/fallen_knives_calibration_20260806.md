...
The single largest defect this run identified is **documented and not fixed.** BTC reports narrated a live, filled *"10% Phase 1A @ ~$65K"* position and mark-to-marketed it across **nine consecutive reports**; ETH narrated *"HOLD ~5% @ ~$1,844"* across **seven**. The ledger shows dust in both assets and no Fallen Knives deal tag anywhere. Two tunes were written to close exactly this hole — `Fill corroboration: LEDGER-CONFIRMED vs UNVERIFIED` and the `Fill-corroboration clock` — and **both were withheld at pre-apply**, for good reasons (they legislate the same object with incompatible state machines; one forbids anchoring a stop, which is a protection removal; both require toolchain edits a SKILL-only apply cannot ship). The framework therefore enters the next cycle with the phantom-fill defect fully described in its own revision log and structurally unaddressed. Nothing in the five applied tunes prevents it recurring.
...
**They cannot both ship, and neither ships alone.** That is the honest reason the largest defect in this corpus goes unfixed — not that it was judged unimportant.
...
And the sentence that should survive the rest of this memo: **the largest defect this run found — nine BTC reports and seven ETH reports mark-to-marketing a position the ledger cannot see — is documented and not fixed.**
