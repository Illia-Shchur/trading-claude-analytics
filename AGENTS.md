...
`finalize-report.mjs`, render deterministic full Markdown with
`render-report.mjs`, then pair-lint JSON and Markdown. JSON is canonical and
...
Node scripts that make report numbers computed instead of narrated. The v2 contract uses the pinned dependencies in `package.json`; arithmetic helpers remain dependency-free. See `tools/README.md` and the "Deterministic toolchain" sections in the framework SKILLs (which are authoritative for what is mandatory):
