...
8. **A fresh ledger is the position of record** *(added 2026-07-28)*. `./bin/analytics position <asset>` reads `position-snapshot.json`, exported from the personal-accounting ledger and derived from actual Binance fills. When **FRESH** (≤12 h), its figures — quantity, ACB cost basis, unrealized and realized PnL, dry powder, open futures positions, fills, closed round-trips and performance stats — are stated as fact and **supersede any narrated position figure carried forward from a prior report.** Hard Rule 4's dry-powder default governs only the absence of fresh evidence.

   The producer now writes dated historical files (`position-snapshot-YYYY-MM-DD_HH-mm-ss-SSSZ.json`) and `position.mjs` selects the newest one from `exports/` by default; the legacy filename remains a migration fallback. Two carve-outs, and only two. **(a) Prices:** snapshot marks are informational and never become canonical spot — Hard Rule 1's independent multi-venue cross-check stands, and sourcing spot from your own database would defeat it. **(b) Phase attribution:** the ledger knows what you hold, not which tranche authorized it; attribution comes from deal tags, and an untagged holding is reported as real-but-`UNTAGGED` rather than guessed into a phase.
...
New reports are JSON-first: finalize `report-machine/2` drafts with
`./bin/analytics finalize-report .report-run/draft.json --out reports/<stem>.json`,
render the full view with `./bin/analytics render-report reports/<stem>.json --mode full --out reports/<stem>.md`, then pair-lint JSON and Markdown. JSON is
canonical; drafts and summaries stay under gitignored `.report-run/`.

## Deterministic Java tooling

Spring commands that make report numbers computed instead of narrated. See `docs/JAVA-MIGRATION.md` and the "Deterministic toolchain" sections in the framework SKILLs (which are authoritative for what is mandatory):
...
- `./mvnw -q -pl analytics-core test` — regression vectors; run before calibrations and after any computation-core change. A SKILL band change and its Java core + tests change land in the same commit.
