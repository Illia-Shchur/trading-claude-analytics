...
await assert.rejects(() => fetchBinanceFundingEvents({ asset: 'eth', fetchImpl: fake([{ symbol: 'BTCUSDT', fundingTime: 8, fundingRate: '0', markPrice: '3000' }]) }), /symbol BTCUSDT does not match requested symbol ETHUSDT/)
await assert.rejects(() => fetchBinanceFundingEvents({ asset: 'eth', fetchImpl: fake([{ fundingTime: 8, fundingRate: '0', markPrice: '3000' }]) }), /symbol <blank> does not match requested symbol ETHUSDT/)
...
const mismatchedMetricsCsv = `${metricsHeader}\n${metricsStart},ETHUSDT,100,200,1.1,1.2,1.3,1.4\n`
assert.throws(() => parseBinanceMetricsArchive(storedZip('BTCUSDT-metrics-mismatched-symbol.csv', mismatchedMetricsCsv), { asset: 'btc', symbol: 'BTCUSDT', startTime: metricsStart, endTime: metricsStart }), /symbol ETHUSDT does not match requested symbol BTCUSDT/)
const blankMetricsCsv = `${metricsHeader}\n${metricsStart},,100,200,1.1,1.2,1.3,1.4\n`
assert.throws(() => parseBinanceMetricsArchive(storedZip('BTCUSDT-metrics-blank-symbol.csv', blankMetricsCsv), { asset: 'btc', symbol: 'BTCUSDT', startTime: metricsStart, endTime: metricsStart }), /symbol <blank> does not match requested symbol BTCUSDT/)
...
const quotedMetricsCsv = `${metricsHeader}\n"${metricsStart}","BTCUSDT","","201","1.1","1.2","1.3","1.4"\n`
const parsedQuotedMetrics = parseBinanceMetricsArchive(storedZip('BTCUSDT-metrics-quoted.csv', quotedMetricsCsv), { asset: 'btc', symbol: 'BTCUSDT', startTime: metricsStart, endTime: metricsStart })
assert.equal(parsedQuotedMetrics.rows[0].open_interest, null)
assert.equal(parsedQuotedMetrics.rows[0].open_interest_value, 201)
assert.equal(parsedQuotedMetrics.rows[0].symbol, 'BTCUSDT')
const malformedQuotedMetrics = `${metricsHeader}\n"${metricsStart}","BTCUSDT","not-a-number",201,1.1,1.2,1.3,1.4\n`
assert.throws(() => parseBinanceMetricsArchive(storedZip('BTCUSDT-metrics-malformed-quoted.csv', malformedQuotedMetrics), { asset: 'btc', symbol: 'BTCUSDT', startTime: metricsStart, endTime: metricsStart }), /non-numeric metric value/)
const malformedCsvQuote = `${metricsHeader}\n"${metricsStart}","BTCUSDT","100"oops,201,1.1,1.2,1.3,1.4\n`
assert.throws(() => parseBinanceMetricsArchive(storedZip('BTCUSDT-metrics-malformed-quote.csv', malformedCsvQuote), { asset: 'btc', symbol: 'BTCUSDT', startTime: metricsStart, endTime: metricsStart }), /closing quote/)
const missingRequiredHeader = metricsHeader.split(',').filter(field => field !== 'sum_open_interest').join(',')
assert.throws(() => parseBinanceMetricsArchive(storedZip('BTCUSDT-metrics-missing-required.csv', `${missingRequiredHeader}\n${metricsStart},BTCUSDT,201,1.1,1.2,1.3,1.4\n`), { asset: 'btc', symbol: 'BTCUSDT', startTime: metricsStart, endTime: metricsStart }), /missing required field sum_open_interest/)
