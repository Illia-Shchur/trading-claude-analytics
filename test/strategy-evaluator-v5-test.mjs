...
const notEdge = { not: { predictor_id: 'edge', op: 'GTE', value: { $gene: 'threshold' } } }
assert.equal(evaluateSignalPredicateV5(notEdge, {}, { threshold: 1 }), false, 'NOT cannot turn a missing predictor into an intent')
assert.equal(evaluateSignalPredicateV5(notEdge, { edge: null }, { threshold: 1 }), false, 'NOT cannot turn a null predictor into an intent')
assert.equal(evaluateSignalPredicateV5(notEdge, { edge: 0 }, { threshold: 1 }), true)
assert.equal(evaluateSignalPredicateV5(notEdge, { edge: 2 }, { threshold: 1 }), false)
...
assert.throws(() => makeRows({ featureRow: { ...feature, edge: undefined } })({ artifact: signalView, episode_ids: ['episode-1'], chromosome: { threshold: 1 }, phase: 'TRAIN_ONLY', fold_id: 'fold-1', cutoff: trainingCutoff }), /missing required predictor fields: edge/)
assert.throws(() => makeRows({ featureRow: { ...feature, edge: null } })({ artifact: signalView, episode_ids: ['episode-1'], chromosome: { threshold: 1 }, phase: 'TRAIN_ONLY', fold_id: 'fold-1', cutoff: trainingCutoff }), /missing required predictor fields: edge/)
const forcedSignal = make(label)({ artifact: signalView, episode_ids: ['episode-1'], chromosome: { threshold: 3 }, forced_intents: { 'episode-1': true }, phase: 'TRAIN_ONLY', fold_id: 'fold-1', cutoff: trainingCutoff })
assert.equal(forcedSignal.signal_intent_vector[0].intent, true, 'forced intents override predicate evaluation for a complete eligible feature')
