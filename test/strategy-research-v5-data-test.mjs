...
test('authoritative acquisition repairs funding marks and produces a reopenable price/funding feature source', async () => {
  const eventAt = Date.parse('2026-01-01T00:00:00.000Z'); const eventAtNext = eventAt + 1_008 * 3_600_000; const fundingTemplate = PLAN.series.find(series => series.series_type === 'funding_events' && series.asset === 'btc'); const fundingSeries = { ...fundingTemplate, start_at: new Date(eventAt).toISOString(), end_at: new Date(eventAtNext).toISOString(), availability_cutoff_at: new Date(eventAtNext + 60_000).toISOString(), cadence_segments: [], event_sequence_mode: true, event_driven: true, expected_step_ms: null, expected_event_count: null }; const signalTemplate = PLAN.series.find(series => series.series_type === 'signal_bars' && series.asset === 'btc' && series.instrument === 'BINANCE_SPOT'); const signalSeries = { ...signalTemplate, start_at: new Date(eventAt).toISOString(), end_at: new Date(eventAtNext).toISOString(), availability_cutoff_at: new Date(eventAtNext + FOUR_HOURS).toISOString(), expected_event_count: 253 }; const aaveSignalSeries = { ...PLAN.series.find(series => series.series_type === 'signal_bars' && series.asset === 'aave' && series.instrument === 'BINANCE_SPOT'), start_at: new Date(eventAt).toISOString(), end_at: new Date(eventAtNext).toISOString(), availability_cutoff_at: new Date(eventAtNext + FOUR_HOURS).toISOString(), expected_event_count: 253 }; const markSeries = PLAN.series.filter(series => series.series_type === 'mark_bars').map(series => ({ ...series, start_at: new Date(eventAt).toISOString(), end_at: new Date(eventAtNext).toISOString(), availability_cutoff_at: new Date(eventAtNext + FOUR_HOURS).toISOString(), expected_event_count: 253 })); const miniPlan = withHash({ ...PLAN, series: [signalSeries, aaveSignalSeries, fundingSeries, ...markSeries] }); const capturedAt = '2026-08-24T12:00:00.000Z'; let markMode = 'MULTI_PAGE'; let calls = 0
  const fetchImpl = async requestUrl => { calls++; const url = new URL(requestUrl); const start = Number(url.searchParams.get('startTime') || eventAt); const path = url.pathname; let payload = []; if (path.endsWith('/fundingRate')) payload = start > eventAtNext ? [] : Array.from({ length: 127 }, (_, index) => ({ symbol: url.searchParams.get('symbol'), fundingTime: eventAt + 1 + index * 8 * 3_600_000, fundingRate: '0.001', markPrice: '' })); else if (path.endsWith('/markPriceKlines')) { if (markMode === 'MISSING') payload = []; else { const interval = url.searchParams.get('interval'); const step = interval === '1h' ? 3_600_000 : 4 * 3_600_000; const first = interval === '1h' ? (start <= eventAt ? eventAt : start) : start; const count = interval === '1h' ? 1000 : 253; payload = Array.from({ length: count }, (_, index) => { const time = first + index * step; const price = markMode === 'MUTATED' && index === 0 ? 999 : 123; return [time, String(price), String(price + 1), String(price - 1), String(price), '1', time + step - 1] }) } } else if (path.endsWith('/klines')) { const step = FOUR_HOURS; payload = Array.from({ length: 253 }, (_, index) => { const time = eventAt + index * step; const close = 100 + index / 100; return [time, '100', String(Math.max(102, close + 1)), '99', String(close), '1', time + step - 1] }) } const body = Buffer.from(JSON.stringify(payload)); return { ok: true, status: 200, headers: { get: name => String(name).toLowerCase() === 'date' ? capturedAt : null }, arrayBuffer: async () => body } }
  const root = mkdtempSync(join(tmpdir(), 'v5-funding-fallback-e2e-')); const first = await acquireAuthoritativeStaging({ plan: miniPlan, outputRoot: root, outputRootReference: 'portable-funding-e2e', fetchImpl, maxPages: 4, maxRows: 10_000, capturedAt, fixtureOnly: true }); assert.equal(first.status, 'STAGING_COMPLETE', JSON.stringify(first.captures.map(capture => ({ type: capture.series_type, asset: capture.asset, complete: capture.coverage?.complete, reason: capture.coverage?.reason, limitations: capture.limitations })))); const fundingCapture = first.captures.find(capture => capture.series_type === 'funding_events'); assert.equal(fundingCapture.coverage.settlement_mark_source, 'BINANCE_MARK_PRICE_KLINE_OPEN_AT_SETTLEMENT'); assert.equal(fundingCapture.partition.storage_role, 'STAGING'); const fundingRows = JSON.parse(`[${readFileSync(join(root, fundingCapture.partition.path), 'utf8').trim().replaceAll('\n', ',')}]`); assert.equal(fundingRows.length, 127); assert.equal(fundingRows[0].settlement_mark, 123); assert.ok(fundingCapture.coverage.settlement_mark_source_response_sha256); const fundingReceipt = JSON.parse(readFileSync(join(root, fundingCapture.source_receipts[0].path), 'utf8')); const markPages = fundingReceipt.pagination.filter(page => page.interval === '1h' && page.response_sha256); assert.ok(markPages.length >= 2); const pageFor = row => markPages.find(page => Number(page.cursor) <= Date.parse(row.settlement_mark_event_time) && Date.parse(row.settlement_mark_event_time) < Number(page.cursor) + Number(page.row_count) * 3_600_000); assert.equal(fundingRows.every(row => row.settlement_mark_source_response_sha256 === pageFor(row)?.response_sha256), true); const parquetRoot = mkdtempSync(join(tmpdir(), 'v5-funding-fallback-parquet-')); const converted = await convertToParquet({ stagingManifest: first, stagingRoot: root, outputRoot: parquetRoot, outputRootReference: 'portable-funding-parquet', fixtureOnly: true }); assert.equal(await verifyParquetConversionManifestAuthoritative(converted, { root: parquetRoot, stagingRoot: root, planSha256: miniPlan.content_sha256 }), true); await assert.rejects(() => verifyParquetConversionManifestAuthoritative(converted, { root: parquetRoot, planSha256: miniPlan.content_sha256 }), /requires the physical acquisition\/staging root/); await assert.rejects(() => verifyParquetConversionManifestAuthoritative(converted, { root: parquetRoot, stagingRoot: mkdtempSync(join(tmpdir(), 'v5-funding-fallback-wrong-staging-')), planSha256: miniPlan.content_sha256 }), /source receipt is missing|reopened funding source receipt/); const firstHash = first.content_sha256; const callsAfterFirst = calls; const resumed = await acquireAuthoritativeStaging({ plan: miniPlan, outputRoot: root, outputRootReference: 'portable-funding-e2e', fetchImpl: async () => { throw new Error('resumed acquisition must reopen custody before refetch') }, maxPages: 4, maxRows: 10_000, capturedAt, fixtureOnly: true }); assert.equal(resumed.content_sha256, firstHash); assert.equal(calls, callsAfterFirst)
  const btcSpotCapture = first.captures.find(capture => capture.asset === 'btc' && capture.instrument === 'BINANCE_SPOT'); const aaveSpotCapture = first.captures.find(capture => capture.asset === 'aave' && capture.instrument === 'BINANCE_SPOT')
  const mutateCapture = transform => withHash({ ...first, captures: first.captures.map(capture => capture === aaveSpotCapture ? transform(capture) : capture) })
  assert.throws(() => verifyAuthoritativeStaging({ manifest: mutateCapture(capture => ({ ...capture, partition: btcSpotCapture.partition })), root, plan: miniPlan, planSha256: miniPlan.content_sha256, allowFixture: true }), /partition row.*(asset|symbol|instrument).*differs/i)
  assert.throws(() => verifyAuthoritativeStaging({ manifest: mutateCapture(capture => ({ ...capture, series_sha256: H })), root, plan: miniPlan, planSha256: miniPlan.content_sha256, allowFixture: true }), /series SHA differs from the frozen plan/i)
  assert.throws(() => verifyAuthoritativeStaging({ manifest: mutateCapture(capture => ({ ...capture, partition: { ...capture.partition, row_count: capture.partition.row_count + 1 } })), root, plan: miniPlan, planSha256: miniPlan.content_sha256, allowFixture: true }), /row_count differs from physical rows/i)
  assert.throws(() => verifyAuthoritativeStaging({ manifest: mutateCapture(capture => ({ ...capture, source_receipts: btcSpotCapture.source_receipts })), root, plan: miniPlan, planSha256: miniPlan.content_sha256, allowFixture: true }), /normalized receipt (series SHA|asset|symbol|instrument).*not bound/i)
  const attackedPath = join(root, aaveSpotCapture.partition.path); const attackedOriginal = readFileSync(attackedPath); const attackedRows = attackedOriginal.toString('utf8').trim().split('\n').map(JSON.parse).map(row => ({ ...row, open: row.open * 2, high: row.high * 2, low: row.low * 2, close: row.close * 2 })); const attackedBytes = Buffer.from(`${attackedRows.map(row => JSON.stringify(row)).join('\n')}\n`); writeFileSync(attackedPath, attackedBytes); const attackedCapture = { ...aaveSpotCapture, partition: { ...aaveSpotCapture.partition, sha256: hash(attackedBytes), bytes: attackedBytes.byteLength } }; const attackedManifest = withHash({ ...first, captures: first.captures.map(capture => capture === aaveSpotCapture ? attackedCapture : capture) }); assert.throws(() => verifyAuthoritativeStaging({ manifest: attackedManifest, root, plan: miniPlan, planSha256: miniPlan.content_sha256, allowFixture: true }), /differs from retained raw response bytes/i); writeFileSync(attackedPath, attackedOriginal)
  ...
  const fundingRegistry = makePredictorRegistry({ predictors: [{ id: 'funding_signal', scalar_type: 'number', source_field: 'funding_rate', source_family: 'funding_events', source_timeframe: 'event', lookback_ms: 86_400_000, availability_derivation: 'latest_exact_settlement_before_completed_bar', code_sha256: H, config_sha256: H, pit_role: 'PREDICTOR', trade_scope: 'CONTEXT_ONLY', recipe: { module: 'builtin-pit-transform/1', kind: 'FIELD', source_field: 'funding_rate', source_series: 'funding_events', required_series_types: ['funding_events'], lookback_bars: 0, min_history: 1, window_policy: 'COMPLETED_OBSERVATIONS_ONLY', availability_policy: 'MAX_INPUT_AVAILABILITY', series_scope: 'SAME_ASSET_FUNDING_SERIES', asof_policy: 'LATEST_AVAILABLE_STRICTLY_BEFORE_DECISION', max_staleness_ms: 86_400_000, lag_bars: 0, resample_policy: 'LAST_AVAILABLE', context_only: true, current_observation_policy: 'INCLUDE_CURRENT_COMPLETED', excluded_window_bars: 0, module_code_sha256: H, module_config_sha256: H } }] })
  const featurePrecommit = withHash({ schema: 'strategy-v5-precommit-fixture/1', version: 1, status: 'FROZEN', name: 'price-funding-feature-source', tradable_instrument_contract: { universe: 'CRYPTO_ONLY', instruments: ['spot'] }, trade_assets: ['btc'] })
  const featureRequirements = makeTimeframeRequirementsFromPredictorRegistry({ predictorRegistry: fundingRegistry, precommitSha256: featurePrecommit.content_sha256 })
  const producedFeatures = produceAuthoritativeFeatureSource({ root, rootReference: 'portable-funding-e2e', plan: miniPlan, acquisition: first, predictorRegistry: fundingRegistry, precommit: featurePrecommit, timeframeRequirements: featureRequirements, fixtureOnly: true })
  validateContractSchema(producedFeatures.manifest)
  const reopened = validateAuthoritativeFeatureSource({ manifest: producedFeatures.manifest, root, expectedPlanSha256: miniPlan.content_sha256, expectedAcquisitionSha256: first.content_sha256, expectedPredictorRegistrySha256: fundingRegistry.content_sha256, expectedPrecommitSha256: featurePrecommit.content_sha256, allowFixture: true })
  assert.equal(reopened.rows.length, 253)
  assert.equal(reopened.rows[0].funding_signal, 0.001, 'the first completed 4h boundary may use the earlier 00:00 settlement')
  const featurePath = join(root, producedFeatures.manifest.artifact.path); const originalFeatureBytes = readFileSync(featurePath)
  writeFileSync(featurePath, Buffer.concat([originalFeatureBytes, Buffer.from('{}\n')]))
  assert.throws(() => validateAuthoritativeFeatureSource({ manifest: producedFeatures.manifest, root, allowFixture: true }), /feature-source artifact|tampered|differs/i)
...
test('overlapping one-minute hydration is merged and portable checkpoint resume is tamper-evident', async () => {
  ...
  const manifest = await hydrateOpportunityWindowsV5({ planSha256: H, candidateSetSha256: B, opportunityEnvelope: makeOpportunityEnvelope({ planSha256: H, candidateSetSha256: B, maxLifecycleMs: 5 * 60_000, windows: [{ asset: 'btc', instrument: 'BINANCE_SPOT', symbol: 'BTCUSDT', window_id: 'w1', execution_start: t(0), execution_end: t(2) }, { asset: 'btc', instrument: 'BINANCE_SPOT', symbol: 'BTCUSDT', window_id: 'w2', execution_start: t(2), execution_end: t(4) }] }), outputRoot: root, outputRootReference: 'portable-v5', capturedAt: CAPTURED, fetchImpl, fixtureOnly: true })
  ...
  const resumed = await hydrateOpportunityWindowsV5({ planSha256: H, candidateSetSha256: B, opportunityEnvelope: frozenEnvelope, outputRoot: root, outputRootReference: 'portable-v5', capturedAt: CAPTURED, fetchImpl: async () => { throw new Error('network must not be called on a verified resume') }, fixtureOnly: true })
  ...
  unlinkSync(rawPath)
  let reacquired = 0
  const reacquiredManifest = await hydrateOpportunityWindowsV5({ planSha256: H, candidateSetSha256: B, opportunityEnvelope: frozenEnvelope, outputRoot: root, outputRootReference: 'portable-v5', capturedAt: CAPTURED, fetchImpl: async url => { reacquired++; return fetchImpl(url) }, fixtureOnly: true })
  ...
  await assert.rejects(() => hydrateOpportunityWindowsV5({ planSha256: H, candidateSetSha256: B, opportunityEnvelope: frozenEnvelope, outputRoot: root, outputRootReference: 'portable-v5', capturedAt: CAPTURED, fetchImpl, fixtureOnly: true }), /content-addressed raw response collision|bytes are missing or tampered/)
  ...
  const receiptReacquired = await hydrateOpportunityWindowsV5({ planSha256: H, candidateSetSha256: B, opportunityEnvelope: frozenEnvelope, outputRoot: root, outputRootReference: 'portable-v5', capturedAt: CAPTURED, fetchImpl, fixtureOnly: true }); assert.equal(receiptReacquired.status, 'STAGING_COMPLETE')
  ...
  await assert.rejects(() => hydrateOpportunityWindowsV5({ planSha256: H, candidateSetSha256: B, opportunityEnvelope: frozenEnvelope, outputRoot: root, outputRootReference: 'portable-v5', capturedAt: CAPTURED, fetchImpl, fixtureOnly: true }), /checkpoint JSON is invalid/)
})
...
test('separated artifacts reject future aliases, duplicate IDs, and promote only verified Parquet', async () => {
  ...
  const separatedLinkRoot = mkdtempSync(join(tmpdir(), 'v5-separated-role-link-')); cpSync(root, separatedLinkRoot, { recursive: true }); const separatedFeature = join(separatedLinkRoot, staging.artifacts.feature.path); renameSync(separatedFeature, `${separatedFeature}.real`); symlinkSync(`${staging.artifacts.feature.path.split('/').at(-1)}.real`, separatedFeature); assert.throws(() => verifySeparatedArtifactManifest(staging, { root: separatedLinkRoot, plan: PLAN, predictorRegistry, candidatePredicates: ['momentum_1'] }), /symlink|single-link/i)
  const separatedHardlinkRoot = mkdtempSync(join(tmpdir(), 'v5-separated-role-hardlink-')); cpSync(output, separatedHardlinkRoot, { recursive: true }); const separatedParquet = join(separatedHardlinkRoot, converted.artifacts.feature.path); renameSync(separatedParquet, `${separatedParquet}.real`); linkSync(`${separatedParquet}.real`, separatedParquet); assert.throws(() => verifySeparatedArtifactManifest(converted, { root: separatedHardlinkRoot, plan: PLAN, requireParquet: true, predictorRegistry, candidatePredicates: ['momentum_1'] }), /single-link/i)
...
test('authoritative evaluator delegates partial exits to the normalized lifecycle engine', () => {
  ...
  const evaluatorSpec = { content_sha256: B, precommit_sha256: H, execution_contract: { risk_convention: { mode: 'FIXED_RISK_BUDGET_USD', budget_usd: 10, precommit_sha256: H }, sizing_contract: { mode: 'FIXED_NOTIONAL_USD', notional_usd: 100, precommit_sha256: H } } }
  const boundCandidate = { ...candidate, lifecycle: { ...candidate.lifecycle, sizing: { mode: 'FIXED_NOTIONAL', notional_usd: 100 } }, risk_contract: { mode: 'FIXED_RISK_BUDGET_USD', budget_usd: 10, precommit_sha256: H, evaluator_spec_sha256: B }, sizing_contract: { mode: 'FIXED_NOTIONAL_USD', notional_usd: 100, precommit_sha256: H, evaluator_spec_sha256: B } }
  const boundExecution = { ...spotExecution }; delete boundExecution.risk_contract
  const boundOutcome = deriveBoundExecutionOutcome({ feature: spotFeature, label: spotLabel, execution: boundExecution, candidate: boundCandidate, evaluatorSpec, metadata: {} })
  assert.ok(Math.abs(boundOutcome.quantity - 100 / 99) < 1e-12, 'the frozen $100 sizing contract, not an alternate lifecycle notional, controls quantity')
  assert.equal(boundOutcome.risk_amount_usd, 10)
  assert.equal(boundOutcome.net_r, boundOutcome.net_pnl_usd / 10)
  assert.equal(boundOutcome.risk_denominator, 'FROZEN_FIXED_RISK_BUDGET')
  const exactRiskCandidate = { ...boundCandidate, lifecycle: { max_lifecycle_ms: 2 * ONE_MINUTE, stop: { type: 'PERCENT', value: 0.05 }, target: { type: 'PERCENT', value: 0.01 }, sizing: { mode: 'FIXED_NOTIONAL', notional_usd: 100 } } }
  const exactRiskBars = [{ event_time: t(0), availability_time: t(1), open: 100, high: 101, low: 99, close: 100 }, { event_time: t(1), availability_time: t(2), open: 101, high: 102, low: 100, close: 101 }, { event_time: t(2), availability_time: t(3), open: 102, high: 103, low: 101, close: 102 }]
  const exactRiskOutcome = deriveBoundExecutionOutcome({ feature: spotFeature, label: spotLabel, execution: { ...boundExecution, child_bars: exactRiskBars }, candidate: exactRiskCandidate, evaluatorSpec, metadata: {} })
  assert.equal(exactRiskOutcome.quantity, 1); assert.equal(exactRiskOutcome.net_pnl_usd, 1); assert.equal(exactRiskOutcome.risk_amount_usd, 10); assert.equal(exactRiskOutcome.net_r, 0.1)
  assert.throws(() => deriveBoundExecutionOutcome({ feature: spotFeature, label: spotLabel, execution: { ...boundExecution, risk_contract: { ...boundCandidate.risk_contract, budget_usd: 11 } }, candidate: boundCandidate, evaluatorSpec, metadata: {} }), /risk_contract values conflict/)
  assert.throws(() => deriveBoundExecutionOutcomeAuthoritative({ feature: spotFeature, label: spotLabel, execution: boundExecution, candidate: { ...boundCandidate, risk_contract: undefined }, evaluatorSpec, metadata: {}, fixtureOnly: false }), /requires an evaluator-bound candidate risk_contract/)
  assert.throws(() => deriveBoundExecutionOutcome({ feature: spotFeature, label: spotLabel, execution: boundExecution, candidate: { ...boundCandidate, lifecycle: { ...boundCandidate.lifecycle, sizing: { mode: 'FIXED_NOTIONAL', notional_usd: 1 } } }, evaluatorSpec, metadata: {} }), /sizing disagrees with the evaluator-bound sizing_contract/)
  assert.throws(() => makeEvaluatorSpecV5({ strategyFamily: 'mismatched-lifecycle-sizing', precommitSha256: H, geneSpace: { content_sha256: H, genes: [] }, predictorRegistry, predicate: { predictor_id: 'momentum_1', op: 'GT', value: 0 }, candidateTemplate: { ...candidate, lifecycle: { ...candidate.lifecycle, sizing: { mode: 'FIXED_NOTIONAL', notional_usd: 1 } } }, executionContract: { risk_convention: { mode: 'FIXED_RISK_BUDGET_USD', budget_usd: 10 }, sizing_contract: { mode: 'FIXED_NOTIONAL_USD', notional_usd: 100 } } }), /lifecycle sizing disagrees with the frozen execution sizing contract/)
})
...
test('funding predictors join the same asset perpetual event stream strictly before each price decision', () => {
  const fundingPredictor = (id, extra = {}) => ({ id, scalar_type: 'number', source_field: 'funding_rate', source_family: 'funding_events', source_timeframe: 'event', lookback_ms: 24 * FOUR_HOURS, availability_derivation: 'latest_funding_settlement_strictly_before_decision', code_sha256: H, config_sha256: H, pit_role: 'PREDICTOR', trade_scope: 'CONTEXT_ONLY', recipe: { module: 'builtin-pit-transform/1', kind: 'FIELD', source_field: 'funding_rate', source_series: 'funding_events', lookback_bars: 0, min_history: 1, window_policy: 'COMPLETED_OBSERVATIONS_ONLY', availability_policy: 'MAX_INPUT_AVAILABILITY', series_scope: 'SAME_ASSET_FUNDING_SERIES', asof_policy: 'LATEST_AVAILABLE_STRICTLY_BEFORE_DECISION', max_staleness_ms: 24 * FOUR_HOURS, lag_bars: 0, resample_policy: 'LAST_AVAILABLE', context_only: true, required_series_types: ['funding_events'], current_observation_policy: 'INCLUDE_CURRENT_COMPLETED', excluded_window_bars: 0, module_code_sha256: H, module_config_sha256: H, ...extra } })
  const registry = makePredictorRegistry({ predictors: [fundingPredictor('funding_latest')] })
  const requirements = makeTimeframeRequirementsFromPredictorRegistry({ predictorRegistry: registry, precommitSha256: H })
  assert.deepEqual(requirements.declarations.map(row => ({ predictor_id: row.predictor_id, interval: row.interval, series_types: row.series_types })), [{ predictor_id: 'funding_latest', interval: 'event', series_types: ['funding_events'] }])

  const decision = hour => new Date(Date.parse(T0) + hour * 60 * 60 * 1000).toISOString()
  const price = (assetName, instrument, hour) => ({ asset: assetName, venue: 'BINANCE', instrument, symbol: `${assetName.toUpperCase()}USDT`, series_role: 'PRICE', timeframe: '4h', event_time: decision(hour), decision_time: decision(hour), availability_time: decision(hour), open: 100, high: 101, low: 99, close: 100 })
  const funding = (assetName, eventMs, availabilityMs, rate, suffix, settlementMs = eventMs) => ({ asset: assetName, venue: 'BINANCE', instrument: 'BINANCE_USDM_PERPETUAL', symbol: `${assetName.toUpperCase()}USDT`, series_role: 'FUNDING', timeframe: 'event', event_id: `${assetName}-${suffix}`, event_time: new Date(eventMs).toISOString(), raw_event_time: new Date(eventMs).toISOString(), settlement_slot: new Date(settlementMs).toISOString(), availability_time: new Date(availabilityMs).toISOString(), funding_rate: rate })
  const atEight = Date.parse(decision(8)); const atTwelve = Date.parse(decision(12))
  const primary = [price('btc', 'BINANCE_SPOT', 8), price('btc', 'BINANCE_USDM_PERPETUAL', 12), price('eth', 'BINANCE_USDM_PERPETUAL', 8)]
  const context = [
    funding('btc', atEight - ONE_MINUTE, atEight - ONE_MINUTE, 0.001, 'before'),
    funding('btc', atEight - 1, atEight - 1, 0.009, 'negative-jitter-same-slot', atEight),
    funding('btc', atTwelve + ONE_MINUTE, atTwelve + ONE_MINUTE, 9, 'future'),
    funding('eth', atEight - ONE_MINUTE, atEight - ONE_MINUTE, -0.002, 'before'),
    funding('eth', atEight - 2 * ONE_MINUTE, atEight, -9, 'available-at-decision'),
  ]
  const primaryCaptureByRow = new Map(primary.map(row => [row, { interval: '4h', series_type: 'signal_bars', series_role: 'PRICE' }]))
  const contextCaptureByRow = new Map(context.map(row => [row, { interval: 'event', series_type: 'funding_events', series_role: 'FUNDING' }]))
  const rows = deriveFeatureRowsFromRaw(primary, { predictorRegistry: registry, captureByRow: primaryCaptureByRow, contextRows: context, contextCaptureByRow })
  assert.equal(rows.length, primary.length, 'funding events are context and never become signal episodes')
  assert.equal(rows.find(row => row.asset === 'btc' && row.instrument === 'BINANCE_SPOT').funding_latest, 0.001, 'same-timestamp funding is unavailable to an exact boundary-open entry')
  assert.equal(rows.find(row => row.asset === 'btc' && row.instrument === 'BINANCE_USDM_PERPETUAL').funding_latest, 0.009)
  assert.equal(rows.find(row => row.asset === 'eth').funding_latest, -0.002, 'late and cross-asset funding cannot enter the feature')

  const staleRegistry = makePredictorRegistry({ predictors: [fundingPredictor('funding_stale', { max_staleness_ms: ONE_MINUTE })] })
  const stale = deriveFeatureRowsFromRaw([primary[1]], { predictorRegistry: staleRegistry, captureByRow: primaryCaptureByRow, contextRows: context, contextCaptureByRow })[0]
  assert.equal(stale.signal_eligible, false); assert.equal(stale.funding_stale, null)
  const slotStaleRegistry = makePredictorRegistry({ predictors: [fundingPredictor('funding_slot_stale', { max_staleness_ms: FOUR_HOURS - 1 })] })
  const slotStaleContext = [funding('btc', atEight + 1, atEight + 1, 0.02, 'positive-jitter-old-slot', atEight)]
  const slotStaleCapture = new Map(slotStaleContext.map(row => [row, { interval: 'event', series_type: 'funding_events', series_role: 'FUNDING' }]))
  const slotStale = deriveFeatureRowsFromRaw([primary[1]], { predictorRegistry: slotStaleRegistry, captureByRow: primaryCaptureByRow, contextRows: slotStaleContext, contextCaptureByRow: slotStaleCapture })[0]
  assert.equal(slotStale.signal_eligible, false); assert.equal(slotStale.funding_slot_stale, null, 'funding staleness is measured from the canonical settlement slot, not raw timestamp jitter')
  const laggedRegistry = makePredictorRegistry({ predictors: [fundingPredictor('funding_lagged', { lag_bars: 1 })] })
  const lagged = deriveFeatureRowsFromRaw([primary[1]], { predictorRegistry: laggedRegistry, captureByRow: primaryCaptureByRow, contextRows: context, contextCaptureByRow })[0]
  assert.equal(lagged.funding_lagged, 0.001)
  assert.throws(() => makePredictorRegistry({ predictors: [fundingPredictor('unsafe_funding', { asof_policy: 'LATEST_AVAILABLE_NOT_AFTER_DECISION' })] }), /funding reference timing contract|schema validation/i)
})
