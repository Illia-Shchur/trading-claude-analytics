...
test('opportunity envelope is predicate superset, half-open, and physically shared', () => {
  ...
  const ineligibleFunding = { asset: 'btc', instrument: 'BINANCE_SPOT', symbol: 'BTCUSDT', decision_time: iso(9), availability_time: iso(9), signal_eligible: false, funding_signal: null }
  assert.throws(() => makeOpportunityEnvelopeV5({ fixtureOnly: true, featureRows: [ineligibleFunding], geneSpace: { genes: [{ name: 'floor', type: 'continuous', min: 0, max: 1 }] }, predicate: { predictor_id: 'funding_signal', op: 'GTE', value: { $gene: 'floor' } }, max_lifecycle_ms: 180_000 }), /no physical decision boundaries/i)
  const eligibleFunding = { ...ineligibleFunding, decision_time: iso(10), availability_time: iso(10), signal_eligible: true, funding_signal: 0.5 }
  const filteredFundingEnvelope = makeOpportunityEnvelopeV5({ fixtureOnly: true, featureRows: [ineligibleFunding, eligibleFunding], candidates: [{ candidate_id: 'not-null', definition: { predicate: { not: { predictor_id: 'funding_signal', op: 'GTE', value: 1 } } } }], geneSpace: { genes: [{ name: 'floor', type: 'continuous', min: 0, max: 1 }] }, predicate: { predictor_id: 'funding_signal', op: 'GTE', value: { $gene: 'floor' } }, max_lifecycle_ms: 180_000 })
  assert.equal(filteredFundingEnvelope.windows.length, 1)
  assert.equal(filteredFundingEnvelope.windows[0].decision_time, iso(10))
  const numericIn = makeOpportunityEnvelopeV5({ fixtureOnly: true, featureRows: [{ ...featureRows[2], signal_eligible: true }], geneSpace: { genes: [] }, predicate: { predictor_id: 'score', op: 'IN', value: [1, 2] }, max_lifecycle_ms: 180_000 })
  assert.equal(numericIn.windows.length, 1)
  const booleanIn = makeOpportunityEnvelopeV5({ fixtureOnly: true, featureRows: [{ ...featureRows[0], signal_eligible: true, armed: false }], geneSpace: { genes: [] }, predicate: { predictor_id: 'armed', op: 'IN', value: [false] }, max_lifecycle_ms: 180_000 })
  assert.equal(booleanIn.windows.length, 1)
  assert.throws(() => makeOpportunityEnvelopeV5({ fixtureOnly: true, featureRows, geneSpace: { genes: [{ name: 'membership', type: 'categorical', values: [[1, 2]], default: [1, 2] }] }, predicate: { predictor_id: 'score', op: 'IN', value: { $gene: 'membership' } }, max_lifecycle_ms: 180_000 }), /gene-controlled IN predicates are unsupported/)
