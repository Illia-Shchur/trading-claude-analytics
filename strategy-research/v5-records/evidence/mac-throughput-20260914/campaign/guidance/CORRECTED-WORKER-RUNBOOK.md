...
[2026-09-14 calculation performance report](MAC-CALCULATION-PERFORMANCE-20260914.md)
and its self-hashed [selected local runtime profile](../strategy-research/v5-records/evidence/mac-throughput-20260914/selected-local-runtime-profile.json)
for the frozen package, stage receipts, and limits.

The selected setting for the tested Mac is 6 outer workers × 1536 MiB per
worker, `-XX:ActiveProcessorCount=1`, and G1. It was selected after four
validated 6-worker runs and two validated 4-worker runs: their mean batch times
differed by 7.426 seconds, smaller than the 9.975-second 4-worker run spread.
The 4-worker runs ranged from 1329.568 to 1339.543 seconds, and the 6-worker
runs from 1339.463 to 1345.565 seconds, overlapping by 0.080 seconds. The plan
treats their means as practically indistinguishable at this measurement scale. The
8-worker × 1280 MiB stage was censored after one of eight jobs completed; that
lower bound is not a candidate. Keep AC power and `caffeinate -is` for timing.
RSS and GC observations remain diagnostic; this profile is not the formal
production or qualification profile. The Java code is platform-neutral, but
the harness and selected profile have not been validated on Windows.

The report's fresh same-setting comparison used the pre-change package at
4 workers × 1536 MiB (1921.886 seconds) and the new package at the same setting
