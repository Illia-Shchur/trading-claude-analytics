...
def runtime_profile_bindings(path: Path, expected_manifest_sha256: str | None = None,
                             expected_profile_sha256: str | None = None) -> dict:
    """Validate and return the machine-readable local worker settings."""
    raw = path.read_bytes()
    try:
        profile = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise RuntimeError(f"local runtime profile is not valid UTF-8 JSON: {path}") from error
    if not isinstance(profile, dict):
        raise RuntimeError(f"local runtime profile must be a JSON object: {path}")
    body = {key: value for key, value in profile.items() if key != "content_sha256"}
    digest = canonical_hash(body)
    if (profile.get("schema") != "mac-local-engineering-runtime/1"
            or profile.get("content_sha256") != digest
            or profile.get("scope") != "LOCAL_ENGINEERING_ONLY"
            or profile.get("outer_workers") not in range(1, 9)
            or profile.get("xmx_mib") not in (512, 640, 768, 896, 1024, 1152, 1280, 1408, 1536, 1664, 1792)
            or profile.get("active_processor_count") != 1
            or profile.get("gc_collector") != "G1"):
        raise RuntimeError(f"local runtime profile failed schema, settings, or own-hash validation: {path}")
    if (expected_manifest_sha256 is not None
            and profile.get("executor_manifest_sha256") != expected_manifest_sha256):
        raise RuntimeError("local runtime profile is not bound to this frozen executor package")
    if expected_profile_sha256 is not None and digest != expected_profile_sha256:
        raise RuntimeError("local runtime profile content hash differs from the controller's frozen binding")
    return {"profile": profile, "content_sha256": digest, "byte_sha256": hashlib.sha256(raw).hexdigest(),
...
def run_one(slot: dict, wave: str, run_root: Path, jar: Path, plan: dict, profile: dict,
            ...
            launch_heap_mib: int, active_processors: int, gc_collector: str,
    ...
    try:
        ...
        gc_flags = {"G1": "-XX:+UseG1GC"}
        if gc_collector not in gc_flags:
            raise RuntimeError(f"unsupported local runtime collector: {gc_collector}")
        command = [
            java, "-XX:+ExitOnOutOfMemoryError", gc_flags[gc_collector],
...
def main() -> int:
    ...
    parser.add_argument("--runtime-profile", type=Path,
                        help="self-hashed machine-readable local worker/heap profile")
    parser.add_argument("--expected-runtime-profile-sha256",
                        help="content hash the controller froze before launching workers")
    parser.add_argument("--controller-source", type=Path,
                        help="stage-controller source whose hash is frozen before the batch")
    ...
    parser.add_argument("--heap-mib", type=int,
                        help="actual worker JVM -Xmx; payload/profile identity remains frozen")
    parser.add_argument("--active-processors", type=int, choices=(1,),
                        help="fixed CPU hint for all comparisons")
    parser.add_argument("--gc-collector", choices=("G1",),
                        help="worker JVM collector; current Mac harness supports G1")
    ...
    if args.runtime_profile is not None:
        runtime_path = ((repo / args.runtime_profile).resolve() if not args.runtime_profile.is_absolute()
                        else args.runtime_profile.resolve())
        runtime_binding = runtime_profile_bindings(
            runtime_path, sha256_file(args.manifest.resolve()), args.expected_runtime_profile_sha256)
        runtime = runtime_binding["profile"]
        explicit = (args.workers, args.heap_mib, args.active_processors, args.gc_collector)
        declared = (runtime["outer_workers"], runtime["xmx_mib"],
                    runtime["active_processor_count"], runtime["gc_collector"])
        if any(value is not None and value != declared[index] for index, value in enumerate(explicit)):
            raise RuntimeError("explicit worker settings disagree with the bound local runtime profile")
        args.workers, args.heap_mib, args.active_processors, args.gc_collector = declared
        args.runtime_profile_binding = {
            "path": runtime_binding["path"], "content_sha256": runtime_binding["content_sha256"],
            "byte_sha256": runtime_binding["byte_sha256"],
            "profile_id": runtime.get("profile_id"),
            "executor_manifest_sha256": runtime.get("executor_manifest_sha256"),
        }
    else:
        if args.expected_runtime_profile_sha256 is not None:
            raise RuntimeError("--expected-runtime-profile-sha256 requires --runtime-profile")
        args.workers = args.workers if args.workers is not None else 4
        args.heap_mib = args.heap_mib if args.heap_mib is not None else 1024
        args.active_processors = args.active_processors if args.active_processors is not None else 1
        args.gc_collector = args.gc_collector if args.gc_collector is not None else "G1"
        args.runtime_profile_binding = None
    ...
    if args.runtime_profile is not None:
        integrity_paths["runtime_profile"] = args.runtime_profile.resolve()
    integrity_paths["worker_harness_source"] = Path(__file__).resolve()
    integrity_paths["strict_artifact_validator_source"] = Path(__file__).with_name("StrictArtifactValidator.java").resolve()
    if args.controller_source is not None:
        integrity_paths["stage_controller_source"] = args.controller_source.resolve()
    ...
    write_json(run_root / "declaration.json", {
        ...
        **binding_manifest,
        ...
        "jvm_gc": args.gc_collector, "runtime_profile": args.runtime_profile_binding,
        "rss_policy": "OBSERVATIONAL_ONLY",
    ...
    for wave in waves:
        ...
        try:
            with ThreadPoolExecutor(max_workers=args.workers) as executor:
                futures = [executor.submit(run_one, slot, wave, run_root, jar, plan, profile,
                                           ...
                                           args.active_processors, args.gc_collector,
                                           wave_started, args.max_batch_seconds)
                           for slot in slots]
        ...
        batch_metrics = {
            ...
            "active_processors": args.active_processors, "jvm_gc": args.gc_collector,
            "runtime_profile": args.runtime_profile_binding,
