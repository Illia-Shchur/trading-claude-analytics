...
research results were removed. FR reports may consume only
...
The calibration path is `./bin/analytics swing-calibrate`: it joins completed
...
`strategy-research/` with `./bin/analytics strategy-research`. Definitions are
...
`./bin/analytics finalize-report`, `./bin/analytics render-report`,
`./bin/analytics lint-report`, and `./bin/analytics export-signals`; v1–2 artifacts remain
...
0. **Position (Hard Rule 8):** run `./bin/analytics position <asset>` first. Real dry powder against the 50% book cap, open shorts reconciled against the tranche ledger, and per-channel realized win rate (`FR-A-` vs `FR-B-`) all come from it — see §6 "Position & Performance". Exit-code contract per AGENTS.md / `tools/README.md`.
...
**Machine contract + lint (mandatory).** Historical report-machine/1 and report-machine/2 reports retain their fenced machine block and legacy lint rules. New swing reports use report-machine/3: the canonical JSON sidecar is paired with compact Markdown and has **no embedded machine block or visible tag registry**. Run `./bin/analytics finalize-report`, `./bin/analytics render-report`, `./bin/analytics lint-report`, and `./bin/analytics export-signals`; the v3 linter checks score/trigger/veto/risk arithmetic, the sidecar hash footer, and the removed-section contract. FR's mandatory price/time stops, ratchets, carry vetoes, and Channel B protections remain in the sidecar. A dry or locked v3 setup retains tags only in its sidecar; no tag registry is printed in Markdown.
