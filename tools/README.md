...
`./bin/analytics strategy-research` manages the Git-tracked
...
It delegates simulation to the Java `SwingEngine`, deterministically expands
...
The Java `LegacyResearchV2` owner provides deterministic event/time-block
...
The Java `StrategyPortfolioV5` owner is a separate crypto-only portfolio simulator:
...
./bin/analytics lint-report reports/<stem>.json
./bin/analytics render-report reports/<stem>.json --mode full --out reports/<stem>.md
./bin/analytics render-report reports/<stem>.json --mode summary
./bin/analytics lint-report reports/<stem>.json --markdown reports/<stem>.md
./bin/analytics export-signals --strict
```
...
operative until then and must not be mixed with v3 scores. `./bin/analytics lint-report`
...
Java `SwingScore` is the shared pure implementation of swing-score/1:
...
the two-bar trigger window. `./bin/analytics swing-calibrate` backfills up to three
...
`./bin/analytics swing-engine` separates backfill from evaluation. It consumes
...
./bin/analytics swing-engine build-cache --input features.json \
  --out .report-run/fast-backtest/features.json.gz
./bin/analytics swing-engine build-cache --assets btc,eth --years 3 \
...
./bin/analytics swing-engine run --cache .report-run/fast-backtest/btc-eth-feature-store.json.gz \
...
./bin/analytics swing-candidates > .report-run/fast-backtest/candidates.json
./bin/analytics swing-engine run --cache .report-run/fast-backtest/btc-eth-feature-store.json.gz \
...
./bin/analytics swing-engine benchmark --cache .report-run/fast-backtest/btc-eth-feature-store.json.gz \
  --candidate-count 1000 --out .report-run/fast-backtest/benchmark.json
./bin/analytics swing-engine inspect-trades \
...
For a fixed candidate selected on prior assets, `./bin/analytics swing-cross-validate`
...
`./bin/analytics swing-strategy-cross-validate` additionally merges both directions
...
Validate an emitted artifact with `./bin/analytics lint-swing-calibration
...
`./bin/analytics compute swing-score --legs '<json>' [--flow '<json>']
...
the Java `ReportContract` is the shared schema + semantic validator;
`./bin/analytics finalize-report` is the only v2 publisher; and `./bin/analytics render-report` is the
pure deterministic view generator. `ReportingPipelineNodeOracleTest` covers flat/cold
...
`./bin/analytics position` consumes the newest historical `exports/position-snapshot-*.json` by default. Pass `--file` with a specific file or directory to override selection; the legacy `position-snapshot.json` remains a migration fallback.

### Futures-only snapshot projection and validity

`./bin/analytics position sp500` now resolves an open derivative through
...
timeframe, year, and month. Public adapters live in the Java market-data module
and are exposed by `./bin/analytics public-data-adapters` (Binance spot/linear OHLC, OI, funding,
...
New strategy families use `./bin/analytics strategy-research-v5` and the
