...
export function makeStatisticalArtifactSet({ lineage, candidates, episodes, exposureHead, metadata = {}, allowSubset = false, genesis = false } = {}) {
  ...
  assertLineage(lineage); validateExposureHead(exposureHead); const { ids } = validateCandidateRows(candidates, exposureHead, { allowEmpty: genesis || allowSubset }); if (genesis && (ids.size || exposureHead.entries.length)) fail('genesis artifact must start with an empty candidate set and exposure head')
...
export function validateStatisticalArtifactSet(artifact, { exposureHead = null, allowSubset = false, allowGenesis = false } = {}) {
  ...
  const genesis = allowGenesis === true || artifact.metadata?.artifact_role === 'GENESIS'; const head = exposureHead || null; const { ids } = validateCandidateRows(artifact.candidates, head, { allowEmpty: genesis || allowSubset }); if (genesis && (ids.size || head?.entries?.length)) fail('genesis artifact must start with an empty candidate set and exposure head'); validateEpisodeRows(artifact.episodes, ids)
  if (!allowSubset && head) {
...
function runGeneticSearchV5Legacy({ artifact, geneSpace, trainingEpisodeIds, evaluator, exposureHead, baseline = null, constraints = {}, config = {}, mode = 'AUTHORITATIVE', foldId = 'training', checkpointPath = null } = {}) {
  ...
  const aliases = [...new Set(allHistory.map(row => row.behavior_alias_sha256))].sort(); const nextHead = appendExposureHead({ prior: exposureHead, datasetSha256: artifact.lineage.dataset_sha256, behaviorAliases: aliases, exposureAttemptCount: allHistory.length, observedAt: config.trainingCutoff || null }); const stableAliases = aliases.filter(alias => (seedMembership.get(alias) || []).length >= 2); const selectionScore = value => Number.isFinite(Number(value)) ? Number(value) : -1e12; const order = (a, b) => Math.min(selectionScore(b.fitness.metrics.bootstrap_p20), selectionScore(b.fitness.metrics.weighted_bootstrap_p20)) - Math.min(selectionScore(a.fitness.metrics.bootstrap_p20), selectionScore(a.fitness.metrics.weighted_bootstrap_p20)) || a.behavior_alias_sha256.localeCompare(b.behavior_alias_sha256); const stableConfirmation = confirmation.filter(row => row.fitness.feasible === true && stableAliases.includes(row.behavior_alias_sha256)); const selected = [...stableConfirmation].sort(order)[0] || null
