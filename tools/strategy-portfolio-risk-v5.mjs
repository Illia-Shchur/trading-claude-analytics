...
export function readBoundMarkArtifact({ path, sha256, expectedVenue = null, expectedIntervalMs = null, asOf = null, consumingCutoff = null, allowFixture = false } = {}) {
  ...
  for (const [key, rows] of bySeries) {
    const consumed = rows.map(markConsumptionTime).sort((a, b) => a - b)
    for (let i = 1; i < consumed.length; i++) {
      if (consumed[i] === consumed[i - 1]) throw new Error(`mark series ${key} has duplicate consumption timestamp`)
      if (consumed[i] - consumed[i - 1] !== intervalMs) throw new Error(`mark series ${key} is not dense at its consumption cadence`)
    }
  }
...
// Every mark consumer goes through this point-in-time gate.  Completed bars
// are identified by their open/event_time but become usable at their close
// (availability_time).  The portfolio grid is therefore a grid of *consumed*
// observations, not a grid of bar opens.  This keeps a completed 4h bar out of
// the 00:00 state when it only became available at 04:00, while still allowing
// an execution artifact to supply an exact entry fill at 00:00.  No consumer
// may forward-fill an observation to a timestamp before this boundary.
function markConsumptionTime(row) { return millis(row.availability_time) }
function availableAt(row, timestamp) { return markConsumptionTime(row) <= Number(timestamp) }
function rowsForSeries(artifact, seriesType, asset, symbol = null) {
  return artifact.rows
    .filter(row => row.series_type === seriesType && row.asset === asset && (!symbol || row.symbol === symbol) && Number.isFinite(markConsumptionTime(row)))
    .sort((a, b) => markConsumptionTime(a) - markConsumptionTime(b))
}
function exactMark(artifact, seriesType, asset, symbol, timestamp) {
  return artifact.rows.find(row => row.series_type === seriesType && row.asset === asset && row.symbol === symbol && markConsumptionTime(row) === Number(timestamp) && availableAt(row, timestamp))
}
...
function alignedPnl(accepted, artifact, minCommon) {
  ...
  for (const key of instruments) {
    const [asset, symbol] = key.split('|'); const rows = rowsForSeries(artifact, 'TRADE_MARK', asset, symbol).filter(row => { const t = markConsumptionTime(row); return t >= start && t <= end })
    if (!rows.length) throw new Error(`selected instrument mark series missing for ${key}`)
    byInstrument.set(key, new Map(rows.map(row => [markConsumptionTime(row), row])))
  }
  const first = byInstrument.get(instruments[0]); const times = [...first.keys()].sort((a, b) => a - b); const cadence = Number(artifact.interval_ms)
  // The first mark may be later than an exact execution entry (a completed
  // bar's close is the first usable observation).  The execution fill binds
  // that entry boundary; the grid must only contain observations that can be
  // consumed and must cover the last open lifecycle state.
  if (times.length < 2 || times[0] > start + cadence || times.at(-1) < end) throw new Error('selected PnL grid does not cover the selected lifecycle window')
