...
const SUPPORTED = new Set([
  ...
  'github-deployment-settings-capture/1', 'github-settings-api-receipt/1', 'strategy-readiness-evidence-manifest/1'
])
...
export function buildReadinessAuditV5({ evidence = {}, evidenceManifest = null, generatedAt = new Date().toISOString(), now = Date.now() } = {}) {
  ...
  try {
    ...
    if (!prospectiveExact || !rootVerified || !githubExact || !ledgerPath || !replayPath || !ok(verified, 'githubAttestation') || !ok(verified, 'githubApiReceipt') || !ok(verified, 'githubCycleReceipt') || !ok(verified, 'githubAttestationKeyRegistry') || !driftExact) throw new Error('activation bundle prerequisites are incomplete')
  ...
  dimensions.push(dimension('activation', 'Activation readiness', verified, [check('activation.trust-root-signature', 'root bundle and pinned fingerprint are verified cryptographically', 1, () => rootVerified, pathEvidence('trustRoot')), check('activation.publication-signature', 'signed evidence and decisions are cryptographically verified', 1, () => activationBundleVerified, pathEvidence('prospective'))], [check('activation.deployment-capture', 'deployment audit and GitHub settings are externally verified', 1, () => activationExact && githubExact, [...pathEvidence('activation'), ...pathEvidence('github')]), check('activation.never-self-declared', 'activation has no self-declared bypass', 1, () => activationExact && activation.checks && Object.values(activation.checks).every(value => value === true), pathEvidence('activation'))]))
...
export function signActionsAttestationV5({ fields, privateKeyPem } = {}) {
  ...
  if (!value.public_key_pem || !value.key_id) throw new Error('Actions attestation requires public key and key id')
...
export function verifyActivationBundleV5({ publication, ledgerPath, replayPath, trustRoot, pinnedTrustRootFingerprint, pinnedGenesisFingerprint, previousTrustRoot = null, evidencePaths = {}, githubCapture, githubCapturePath = null, githubCaptureSha256 = null, githubApiReceiptPath = null, githubApiReceiptSha256 = null, githubDriftPath = null, githubDriftSha256 = null, githubCycleReceiptSha256 = null, githubAttestationPath = null, githubAttestationSha256 = null, githubAttestationPublicKeyFingerprint = null, githubAttestationKeyRegistryPath = null, githubAttestationKeyRegistrySha256 = null, githubAttestationKeyRegistryByteSha256 = null, nowAt = Date.now(), expectedLineageSha256 = null } = {}) {
  ...
  const capture = githubCapture?.value || githubCapture; if (!capture || capture.schema !== 'github-deployment-settings-capture/1' || capture.content_sha256 !== ownHash(capture)) throw new Error('physical GitHub settings capture is required'); validateContractSchema(capture)
  ...
  const rulesetAppPolicy = capture.rulesets?.api_status === 200 && capture.rulesets?.protected_ref_matches === true && capture.rulesets?.bypass_verified === true && capture.rulesets?.actions_only_bypass_verified === true && capture.rulesets?.enforcement_verified === true && capture.rulesets?.rules_verified === true
  ...
  if (!capture.verified || capture.repository_visibility_verified !== true || !['PUBLIC', 'PRIVATE'].includes(capture.repository_visibility) || capture.api_response?.provider !== 'github-api' || capture.branch_protection?.verified !== true || (!legacyBranchPolicy && (!capture.rulesets?.verified || !rulesetAppPolicy)) || capture.environment_protection?.verified !== true || capture.environment_protection?.can_admins_bypass === true || capture.settings_token_secret?.verified !== true || capture.settings_token_identity?.verified !== true || capture.oidc_signature_verified !== true || capture.oidc_subject_restricted !== true || capture.blocked_reason) throw new Error('GitHub custody/protection is unavailable; activation remains blocked')
  ...
  if (!githubCycleReceiptSha256 || !HASH.test(String(githubCycleReceiptSha256)) || !Array.isArray(publication?.evidence) || !publication.evidence.some(row => row.sha256 === githubAttestationSha256) || !publication.evidence.some(row => row.sha256 === captureBytesSha256) || !publication.evidence.some(row => row.sha256 === githubApiReceiptSha256) || !publication.evidence.some(row => row.sha256 === githubCycleReceiptSha256) || !publication.evidence.some(row => row.sha256 === registryByteSha256) || !publication.evidence.some(row => row.sha256 === githubDriftSha256)) throw new Error('activation publication evidence inventory omits settings/API/drift/cycle/attestation/key-registry bytes')
