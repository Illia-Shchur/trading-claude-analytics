...
import { ACCEPTANCE_CONTRACT_SCHEMA, EVIDENCE_BUNDLE_V2_SCHEMA, EXPERIMENT_V3_SCHEMA, RUN_V3_SCHEMA, validateAcceptanceContract, validateEvidenceBundleV2, validateExperimentV3, validateRunV3 } from './strategy-research-v3.mjs'
...
export function rebuildIndex(root) {
  ...
  const performance = []
  const v2ByRun = new Map(runV2Records.map(({ value }) => [value.run_id, value])); const v3ByRun = new Map(runV3Records.map(({ value, evidence }) => [value.run_id, evidence]))
  for (const row of runs) { const metricRows = v2ByRun.has(row.run_id) ? (v2ByRun.get(row.run_id)?.metrics || []) : v3ByRun.has(row.run_id) ? (v3ByRun.get(row.run_id)?.metrics || []) : readJSONL(join(root, dirname(row.path), 'metrics.jsonl')); for (const metric of metricRows) { const values = structuredClone(metric.metrics || metric); for (const key of ['run_id', 'experiment_id', 'evidence_phase', 'asset', 'scope', 'candidate_id', 'status']) delete values[key]; performance.push({ run_id: row.run_id, experiment_id: row.experiment_id, evidence_phase: row.evidence_phase, asset: metric.asset, scope: metric.scope || (metric.asset ? 'ASSET' : 'PORTFOLIO'), candidate_id: metric.candidate_id, status: metric.asset ? decisionsByRun.get(row.run_id)?.per_asset.find(x => x.asset === metric.asset)?.status : row.portfolio_status, ...values }) } }
...
export function validateRegistry(root) {
  ...
  for (const path of walk(join(root, 'runs')).filter(path => basename(path) === 'run.json')) try { const value = readJSON(path); if (value.schema === RUN_V2_SCHEMA) { validateV2Document(value); if (basename(dirname(path)) !== value.run_id) throw new Error('v2 run directory name does not match run_id'); const experiment = readJSON(join(root, 'experiments', value.experiment_id, 'experiment.json')); const definition = readJSON(resolve(root, experiment.definition.path)); const candidateSet = readJSON(join(root, 'experiments', value.experiment_id, experiment.candidate_set.path)); const precommit = readJSON(resolve(root, definition.precommit.path)); if (hash(precommit) !== value.precommit_sha256 || hash(definition) !== value.definition_sha256 || hash(experiment) !== value.experiment_sha256 || hash(candidateSet) !== value.candidate_set_sha256) throw new Error('v2 run cross-file hash mismatch'); if (value.provenance === 'AUTHORITATIVE_RECOMPUTED') { const evidencePath = join(root, 'evidence-bundles', `${value.evidence_bundle_sha256}.json`); if (!existsSync(evidencePath)) throw new Error('authoritative run evidence bundle is missing'); const evidence = readJSON(evidencePath); validateEvidenceBundle(evidence, { experiment, candidateSet }); if (basename(evidencePath, '.json') !== evidence.content_sha256 || evidence.content_sha256 !== value.evidence_bundle_sha256) throw new Error('authoritative run evidence bundle hash/path mismatch') } if (value.stage !== experiment.stage || value.evidence_phase !== experiment.evidence_phase || value.strategy_id !== definition.strategy_id || stable(value.required_assets) !== stable(experiment.required_assets)) throw new Error('v2 run duplicated metadata does not match bound definition/experiment') } else if (value.schema === RUN_V3_SCHEMA) { validateRunV3(value); if (basename(dirname(path)) !== value.run_id) throw new Error('v3 run directory name does not match run_id'); if (value.evidence_bundle_sha256) { const evidencePath = join(root, 'evidence-bundles', `${value.evidence_bundle_sha256}.json`); if (!existsSync(evidencePath)) throw new Error('v3 run evidence bundle is missing'); const evidence = readJSON(evidencePath); validateEvidenceBundleV2(evidence); if (evidence.content_sha256 !== value.evidence_bundle_sha256 || evidence.experiment_sha256 !== value.experiment_sha256 || evidence.evidence_phase !== value.evidence_phase || stable(evidence.decisions) !== stable(value.decisions)) throw new Error('v3 run/evidence decision or hash mismatch') } } else validateRunDirectory(dirname(path)) } catch (error) { errors.push(`${relative(root, path)}: ${error.message}`) }
  for (const path of walk(root).filter(path => path.endsWith('.json'))) try { const value = readJSON(path); validateContractSchema(value); if (value.schema === DATA_MANIFEST_SCHEMA) validateDataManifest(value); if (value.schema === EVIDENCE_BUNDLE_SCHEMA) validateEvidenceBundle(value); if (value.schema === ACCEPTANCE_CONTRACT_SCHEMA) validateAcceptanceContract(value); if (value.schema === EXPERIMENT_V3_SCHEMA) validateExperimentV3(value, { acceptance: value.acceptance_contract }); if (value.schema === EVIDENCE_BUNDLE_V2_SCHEMA) validateEvidenceBundleV2(value); if (value.schema === RUN_V3_SCHEMA) validateRunV3(value); if (value.schema === 'strategy-research-index/3') { const copy = structuredClone(value); delete copy.content_sha256; if (value.content_sha256 !== hash(copy)) throw new Error('v3 index content hash mismatch') } if (value.schema && ![REGISTRY_SCHEMA, 'strategy-research-index/3', DEFINITION_SCHEMA, EXPERIMENT_SCHEMA, CANDIDATE_SET_SCHEMA, RUN_SCHEMA, PRECOMMIT_SCHEMA, DEFINITION_V2_SCHEMA, EXPERIMENT_V2_SCHEMA, CANDIDATE_SET_V2_SCHEMA, RUN_V2_SCHEMA, DATA_MANIFEST_SCHEMA, EVIDENCE_BUNDLE_SCHEMA, PORTFOLIO_MARK_PATH_SCHEMA, ACCEPTANCE_CONTRACT_SCHEMA, EXPERIMENT_V3_SCHEMA, EVIDENCE_BUNDLE_V2_SCHEMA, RUN_V3_SCHEMA, 'research-lake-catalog/1', 'research-universe/3', 'strategy-source-registry/1', 'strategy-research-stack/1', 'strategy-execution-policy/1', 'strategy-portfolio-policy/1', 'strategy-source-receipt/1', 'strategy-exposure-ledger/1', 'strategy-prospective-ledger/1', 'strategy-prospective-attestation/1', 'strategy-prospective-gate/1', 'strategy-activation/1', 'strategy-activation-revocation/1', 'strategy-readiness-audit/1', 'strategy-candidate-set/4', 'strategy-execution-result/1', 'strategy-portfolio-result/1', 'strategy-wfo-result/1', 'strategy-stress-result/1', 'prospective-monitoring/2', 'strategy-research-index/4', 'strategy-research-run/4', 'strategy-research-evidence/4'].includes(value.schema)) errors.push(`${relative(root, path)}: unknown schema ${value.schema}`) } catch (error) { errors.push(`${relative(root, path)}: ${error.message}`) }
