...
function ledgerIndex(lineage) { return withHash({ schema: 'strategy-prospective-ledger-index/1', version: 1, lineage_sha256: lineage, sequence: 0, head_sha256: ledgerGenesis(lineage), prior_snapshot_root: null, prior_head_sha256: null, event_refs: [] }) }
...
function ledgerRefSources(path, index, snapshotRootBase = null, seen = new Set()) { const root = resolve(path); if (seen.has(root)) throw new Error('prospective ledger snapshot chain contains a cycle'); seen.add(root); const priorRoot = index.prior_snapshot_root; if (!priorRoot) return [{ path: root, refs: index.event_refs }]; if (!HASH.test(String(priorRoot)) || !HASH.test(String(index.prior_head_sha256 || ''))) throw new Error('prospective ledger snapshot predecessor binding is invalid'); const base = resolve(snapshotRootBase || dirname(dirname(root))); const priorPath = join(base, priorRoot, 'ledger'); const priorIndex = readLedgerIndex(priorPath); if (priorIndex.lineage_sha256 !== index.lineage_sha256 || priorIndex.head_sha256 !== index.prior_head_sha256 || priorIndex.sequence >= index.sequence) throw new Error('prospective ledger snapshot predecessor is not the exact immutable prefix'); return [...ledgerRefSources(priorPath, priorIndex, snapshotRootBase, seen), { path: root, refs: index.event_refs }] }
function loadLedgerEvents(path, index, atSequence = index.sequence, snapshotRootBase = null) { const refs = ledgerRefSources(path, index, snapshotRootBase).flatMap(row => row.refs.map(ref => ({ ...row, ref }))).filter(row => row.ref.sequence <= atSequence); let previous = ledgerGenesis(index.lineage_sha256); const events = []; for (const row of refs) { const ref = row.ref; const physical = physicalJson({ path: eventFile(row.path, ref), sha256: ref.byte_sha256, hashField: 'event_sha256' }); const event = physical.value; if (event.sequence !== ref.sequence || event.event_sha256 !== ref.event_sha256 || event.previous_head_sha256 !== previous || event.event_sha256 !== ownHash(event, 'event_sha256')) throw new Error('immutable prospective event chain is invalid'); previous = event.event_sha256; events.push(event) }; if (refs.length !== atSequence && atSequence !== 0) throw new Error('requested ledger sequence is unavailable'); const head = refs.length ? refs.at(-1).ref.event_sha256 : ledgerGenesis(index.lineage_sha256); return { events, head, sequence: refs.length } }
...
// Hourly scheduling is more frequent than the frozen 4h bar cadence.  A
// repeat is a safe no-op only when the current ledger contains the exact same
// latest SIGNAL identity and every source/decision binding agrees.  Any
// divergence (including a same-ID payload substitution) remains an error.
export function verifyCompletedBarNoOp({ ledger, bar, sourceReceiptSha256, signalDecisionSha256, reservationSha256, candidateSetSha256, evaluatorCodeSha256, featureInputSha256 } = {}) {
  if (!ledger || !bar?.completed_bar_id) return false
  const asset = String(bar.asset || '').toLowerCase(); const matches = ledger.events.filter(row => row.kind === 'SIGNAL' && String(row.asset).toLowerCase() === asset && row.completed_bar_id === String(bar.completed_bar_id))
  if (!matches.length) return false
  if (matches.length !== 1) throw new Error('duplicate completed-bar identity is not a valid no-op')
  const event = matches[0]; const payload = event.payload || {}
  if (event.source_receipt_sha256 !== sourceReceiptSha256 || payload.signal_decision_sha256 !== signalDecisionSha256 || payload.reservation_sha256 !== reservationSha256 || payload.candidate_set_sha256 !== candidateSetSha256 || payload.evaluator_code_sha256 !== evaluatorCodeSha256 || payload.feature_input_sha256 !== featureInputSha256 || event.decision_time !== iso(bar.availability_time) || event.availability_time !== iso(bar.availability_time)) throw new Error('same completed-bar identity has divergent source or signal payload')
  // A later OUTCOME is the normal completion of this same bar and does not
  // make the hourly retry stale.  A newer SIGNAL for the asset does.
  if (ledger.events.some(row => row.kind === 'SIGNAL' && String(row.asset).toLowerCase() === asset && time(row.decision_time) > time(bar.availability_time))) throw new Error('same completed-bar identity is not the latest ledger bar')
  return true
}
